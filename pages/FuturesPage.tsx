...
const VisionCard: React.FC<{ title: string; description: string; icon: React.ReactNode }> = ({ title, description, icon }) => (
    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md p-8 rounded-2xl shadow-xl border border-white/20 dark:border-slate-700 transition-all hover:scale-105 hover:shadow-2xl flex flex-col items-center text-center h-full">
        ...
        <ul className="text-text-light dark:text-slate-300 leading-relaxed text-sm list-disc list-inside text-left w-full space-y-2">
            {/* Split description by sentences for bullet points to improve readability */}
            {description.split('. ').map((sentence, idx) => (
                sentence.trim() && <li key={idx}>{sentence.trim()}{!sentence.endsWith('.') && '.'}</li>
            ))}
        </ul>
    </div>
);
    ...
    return (
        <div>
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 my-16 space-y-24">
                ...
                {/* 2. Planetary Healing (Sustainability) */}
                <section className="bg-green-50/50 dark:bg-green-900/10 rounded-3xl p-8 md:p-12 border border-green-100 dark:border-green-900/30 shadow-sm">
                    <div className="flex flex-col md:flex-row items-center gap-12">
                        <div className="md:w-1/2">
                            <h2 className="text-3xl font-display font-bold text-green-800 dark:text-green-400 mb-6">{t('Sustainability_Title')}</h2>
                            <p className="text-text-light dark:text-slate-300 text-lg mb-6">{t('Sustainability_Desc')}</p>
                            <ul className="space-y-4">
                                {[t('Sus_Goal_1'), t('Sus_Goal_2'), t('Sus_Goal_3')].map((goal, idx) => (
                                    <li key={idx} className="flex items-start">
                                        <svg className="w-6 h-6 text-green-500 mr-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                        <span className="text-text-dark dark:text-slate-200">{goal}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
                        <div className="md:w-1/2">
                            <img src="https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?auto=format&fit=crop&w=800&q=80" alt="Sustainable Future" className="rounded-2xl shadow-xl w-full object-cover h-80" loading="lazy" />
                        </div>
                    </div>
                </section>
